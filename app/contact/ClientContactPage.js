...
function ContactContent({ settings, content }) {
      ...
      <section className={`section ${styles.contactSection}`}>
        <div className={`container ${styles.grid}`}>
          ...
          <div className={`reveal ${styles.infoCol}`}>
            <h2 className="heading-lg" style={{ marginBottom: 'var(--space-6)' }}>{content.sectionHeading}</h2>
            ...
            <div className={styles.infoBlock}>
              ...
              <div>
                ...
                <p>{settings.phone} ({settings.phoneHours})</p>
                <p>{settings.emergencyPhoneLabel}: {settings.emergencyPhone}</p>
              </div>
            </div>
            ...
            <div className={styles.infoBlock}>
              ...
              <div>
                ...
                <p>{settings.email}</p>
                <p>{settings.partnershipsEmail}</p>
              </div>
            </div>
          </div>
...
export default function ClientContactPage({ settings, content }) {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh' }} />}>
      <ContactContent settings={settings} content={content} />
    </Suspense>
  );
}
